...
contract MockCCIPRouter is IRouter, IRouterClient {
        ...
        returns (bytes32)
    {
        ...
        //ARYS: if we're sending a message/tokens to Sepolia consider the source chain to be Mumbai and viceversa
        uint64 _sourceChainSelector;
        if (destinationChainSelector == 16015286601757825753) {
            _sourceChainSelector = 12532609583862916517; // Mumbai
        } else if (destinationChainSelector == 12532609583862916517) {
            _sourceChainSelector = 16015286601757825753; // Sepolia
        } else {
            console.log("This system is for Sepolia-Mumbai cross-chain communication only!");
        }
        ...
        for (uint256 i = 0; i < message.tokenAmounts.length; ++i) {
            ...
            if (destinationChainSelector == 16015286601757825753) {
                console.log("Tokens transferred from Mumbai to Sepolia");
            } else {
                console.log("Tokens transferred from Sepolia to Mumbai");
            }
        }
